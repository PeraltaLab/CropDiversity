...
author: "Ariane L. Peralta, Yanmei Sun, Marshall McDaniel, Jay T. Lennon"
...
# Import OTU data
# Import Raw Data
BDdata.in <- read.otu("../data/Classified_genus_OTUs.shared")

# Remove Mock Community
BDdata.in2 <- BDdata.in[which(rownames(WLdata.in) != "Mock"), ]

# Remove OTUs with less than two occurences across all sites
BDdata <- BDdata.in2[, which(colSums(WLdata.in2) >= 2)]
...
# Plant Data
```{r}
WL.plant <- read.csv("../data/BDveg.csv", row.names = 1)
...
design <- read.csv("../data/BDsoil_exptdesign.csv", row.names = 1)
treatments <- design$Treatment
levels(treatments) <- c("mC", "C1cov", "CS", "CSW", 
                        "CSW1cov", "CSW2cov", "Fallow")
#DO NOT include analysis of "S" and "W" treatments
```
...
row.names(WL.plant) == row.names(design) # check
