...
grp <- TUKEY$groups[c(match(c("C", "C-1cov", "CS", "CSW", "CSW-1cov", 
                            "CSW-2cov", "fallow" ), rownames(TUKEY$M))), ]

plot.data <- ggplot_build(p)$data[[1]]
grp.pos <- aggregate(y ~ x, plot.data, max)$y
grp.tuk <- aggregate(y ~ x, plot.data, max)$M
...
H
  ...
  #annotate("text", x = c(1:7), y = TUKEY$means + 0.02)
...
#library(Rmisc)
#library(ggplot2)
jpeg("../figures/DiversityAll.jpeg", width = 480, height = 480 * 1.75) #doesn't work
multiplot(H, rich, J, cols=1) #unable to save
dev.off()
