...
points <- cbind(as.data.frame(BD_pcoa$points), new.phldREL2$rotation)
L.centroids <- melt(points, id="new.phldREL2$rotation", measure.vars = c("V1", "V2"))
centroids <- cast(L.centroids, variable ~ new.phldREL2$rotation, mean)
centroids.se <- cast(L.centroids, variable ~ new.phldREL2$rotation, se)
centroids.sd <- cast(L.centroids, variable ~ new.phldREL2$rotation, sd)
