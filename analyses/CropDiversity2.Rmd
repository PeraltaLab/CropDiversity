...
phld.test <- lapply(phld[,-c(1:7)], as.numeric)
str(phld.test)

# Make Relative Abundance Matrices ###need to fix Error in 1:dim(phld.test)[1] : argument of length 0
...
adonis = adonis(phld[,-c(1:7)] ~ rotation, method = "bray", data = phld, perm=1000)
adonis
```
...
```{r}
# Create Distance Matrix
sampleREL.dist <- vegdist(phld[,-c(1:9)], method="bray")
